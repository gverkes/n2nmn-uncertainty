...
if __name__ == "__main__":
    ...
    for query in test_queries:
        print(ppp(query))
        if 'triangle' not in pp(query):
            images = gen_images(query)
            if images is None:
                # logging.warn("excluding %s", pp(query))
                continue
            test_data_triangle += images
    ...
    sets = {
        ...
        "val": val_data,
        "test": test_data_triangle
    }

    for set_name, set_data in sets.items():

        set_inputs = np.asarray([image.data[:,:,0:3] for query, image, result in set_data])
        np.save("no_triangle/%s.input" % set_name, set_inputs)

        with open("no_triangle/%s.query" % set_name, "w") as query_f, \
                 open("no_triangle/%s.query_str.txt" % set_name, "w") as query_txt_f, \
                 open("no_triangle/%s.output" % set_name, "w") as output_f:
