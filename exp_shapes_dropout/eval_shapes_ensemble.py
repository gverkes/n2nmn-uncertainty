...
scores_val = np.zeros((N_EVALUATIONS, num_questions, 2))
answer_correct = 0
layout_correct = 0
layout_valid = 0
for n_iter in range(int(num_batches)):
    ...
    scores_val[0, n_begin:n_end, :] = sess.partial_run(h, scores, feed_dict=expr_feed)

    # Needed for succesive partial runs
    expr_feed[text_seq_batch] = text_seq_array[:, n_begin:n_end]
    expr_feed[seq_length_batch] = seq_length_array[n_begin:n_end]
    expr_feed[image_batch] = image_array[n_begin:n_end]

    for i in range(1, N_EVALUATIONS):
        h = sess.partial_run_setup(scores,
        [text_seq_batch, seq_length_batch, image_batch,
         compiler.loom_input_tensor, expr_validity_batch])

        scores_val[i, n_begin:n_end, :] = sess.partial_run(h, scores, feed_dict=expr_feed)

    # compute accuracy
    predictions = np.argmax(np.mean(scores_val[:, n_begin:n_end, :], axis=0), axis=1)
