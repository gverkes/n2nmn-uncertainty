...
if __name__ == "__main__":
    ...
    if NO_TRIANGLE_QUESTIONS:
        tops = ['is_red', 'is_green', 'is_blue', 'is_circle', 'is_square']
    else:
    ...
    if NO_TRIANGLE_QUESTIONS:
        bottoms = ['red', 'green', 'blue', 'circle', 'square']
    else:
    ...
    for set_name, set_data in sets.items():
        save_dir = "{}/../NO_TRI_IMG_{}_NO_TRI_QST_{}/{}".format(os.path.dirname(__file__), 
                                                                 int(NO_TRIANGLE_IMAGES),
                                                                 int(NO_TRIANGLE_QUESTIONS), set_name)
        ...
        np.save("{}.input".format(save_dir), set_inputs)

        with open("{}.query".format(save_dir), "w") as query_f, \
                 open("{}.query_str.txt".format(save_dir), "w") as query_txt_f, \
                 open("{}.output".format(save_dir), "w") as output_f:
            for query, image, result in set_data:
                str_query = pp(query)
                strtxt_query = ppp(query)
                query_f.write(str_query + "\n")
                query_txt_f.write(strtxt_query + "\n")
